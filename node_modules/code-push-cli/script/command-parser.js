...
function removeCollaborator(commandName, yargs) {
    ...
    yargs.usage(USAGE_PREFIX + " collaborator " + commandName + " <appName> <email>")
        .demand(4, 4) // Require exactly four non-option arguments.
...
var argv = yargs.usage(USAGE_PREFIX + " <command>")
    ...
    yargs.usage(USAGE_PREFIX + " access-key <command>")
        ...
        .command("patch", "Update the name and/or TTL of an existing access key", function (yargs) { return accessKeyPatch("patch", yargs); })
    ...
    yargs.usage(USAGE_PREFIX + " app <command>")
        ...
        .command("transfer", "Transfer the ownership of an app to another account", function (yargs) {
    ...
    yargs.usage(USAGE_PREFIX + " deployment <command>")
        ...
        yargs.usage(USAGE_PREFIX + " deployment rename <appName> <currentDeploymentName> <newDeploymentName>")
            .demand(5, 5) // Require exactly five non-option arguments.
    ...
    .command("link", "Link an additional authentication provider (e.g. GitHub) to an existing CodePush account", function (yargs) {
    isValidCommandCategory = true;
    isValidCommand = true;
    yargs.usage(USAGE_PREFIX + " link")
        .demand(1, 2) // Require one non-optional and one optional argument.
        .example("link", "Links an account on the CodePush server")
        .check(function (argv, aliases) { return isValidCommand; }); // Report unrecognized, non-hyphenated command category.
    addCommonConfiguration(yargs);
})
    .command("login", "Authenticate with the CodePush server in order to begin managing your apps", function (yargs) {
    isValidCommandCategory = true;
    isValidCommand = true;
    ...
    .command("register", "Register a new CodePush account", function (yargs) {
    isValidCommandCategory = true;
    isValidCommand = true;
    ...
    .command("session", "View and manage the current login sessions associated with your account", function (yargs) {
    isValidCommandCategory = true;
    yargs.usage(USAGE_PREFIX + " session <command>")
        .demand(2, 2) // Require exactly two non-option arguments.
        .command("remove", "Remove an existing login session", function (yargs) { return sessionRemove("remove", yargs); })
        .command("rm", "Remove an existing login session", function (yargs) { return sessionRemove("rm", yargs); })
        .command("list", "List the current login sessions associated with your account", function (yargs) { return sessionList("list", yargs); })
        .command("ls", "List the current login sessions associated with your account", function (yargs) { return sessionList("ls", yargs); })
        .check(function (argv, aliases) { return isValidCommand; }); // Report unrecognized, non-hyphenated command category.
    addCommonConfiguration(yargs);
})
